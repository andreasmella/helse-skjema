...
const labels = {
    "nb-NO": {
        ...
        personInfo: "Personopplysninger",
        name: "Navn",
        nationalId: "Fødselsnummer (11 siffer)",
        mobilePhone: "Mobiltelefon",
        alternatePhone: "Alternativ telefon"
    },
    ...
    "nn-NO": {
        ...
        personInfo: "Personopplysningar",
        name: "Namn",
        nationalId: "Fødselsnummer (11 siffer)",
        mobilePhone: "Mobiltelefon",
        alternatePhone: "Alternativt telefonnummer"
    },
...
console.log("LABELS:", labels[language]);

const L =
    labels[language] ||
    labels[language?.substring(0,2)] ||
    labels["nb-NO"];
        const paragraphs = [];
...
function flushSectionTable() {

    if (
        !currentSectionTitle ||
        currentSectionRows.length === 0
    ) {
        return;
    }
...
function startSectionTable(title) {
...
function pushConditionBox(text) {
    ...
    paragraphs.push(
        ...
        function getEnableWhenText(it, questionMap) {
            ...
            const answerText =
    rule.answerBoolean === true
        ? "Ja"
        : rule.answerBoolean === false
        ? "Nei"
        : "angitt verdi";
...
return (
    ...
    for (const rule of it.enableWhen) {
...
if (
    ...
    getAgeCondition(it);
                ...
                /*if (conditionalText) {
    pushConditionBox(conditionalText);
}
if (ageCondition) {
    pushConditionBox(ageCondition);
}*/
...
pushQuestion(text);
addShortField();
/*function renderLongQuestion(question) {
    ...
    jsonObj?.resourceType === "Questionnaire"
        ...
        : jsonObj?.entry?.find(
            e =>
                ...
                e.resource?.language === selectedLanguage
        )?.resource;
